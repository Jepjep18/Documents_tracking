                                            ...
                                            <tr>
                                                ...
                                                <td class="py-2">
                                                    @if ($document->acceptance && $document->acceptance->reuploaded_file_name)
                                                        <a href="{{ route('download.reupload', ['file' => $document->acceptance->reuploaded_file_name]) }}"
                                                            class="text-blue-600 hover:text-blue-800 font-semibold download-link"
                                                            download>{{ $document->acceptance->reuploaded_file_name }}</a>
                                                    @else
                                                        <span
                                                            class="@if (!$document->acceptance || !$document->acceptance->reuploaded_file_name) text-red-500 @endif">No
                                                            Reuploaded File</span>
                                                    @endif
                                                </td>
                                                ...
                                                <td class="py-2">
                                                    @if (
                                                        $document->department &&
                                                            ...
                                                            $document->acceptance->reuploaded_file_name &&
                                                            $document->acceptance->updated_at)
                                                        <button
                                                            class="bg-green-500 text-white font-semibold px-4 py-2 rounded">Done</button>
                                                    @else
                                                        <button
                                                            class="bg-red-500 text-white font-semibold px-4 py-2 rounded">Pending</button>
                                                    @endif
                                                </td>
            ...
            <div id="modal"
                ...
                <div class="bg-white p-8 rounded shadow-md w-full md:w-1/2">
                    ...
                    <form action="{{ route('doctrack.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="personnel" class="block text-sm font-medium text-gray-700">Assigned
                                Personnel</label>
                    ...
                    <button id="closeModalButton"
                        class="mt-4 px-4 py-2 bg-blue-500 text-white font-semibold rounded">Close</button>
                </div>
            </div>
